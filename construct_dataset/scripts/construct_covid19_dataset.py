...
from scripts.construct_dataset import *
...
class Construct_covid19_dataset(object):
    ...
    def create_dataset(self): # Code to label the tweets with sentiment labels
        ...
        for i in range(self.config["start"], self.config["stop"]+1):
            ...
            if not os.path.exists("assets/logs/corona_tweets_"+i+"/"):
                os.makedirs("assets/logs/corona_tweets_"+i+"/")
            log_file = open("assets/logs/corona_tweets_"+i+"/"+self.config["asset_name"]+"_"+current_time+".txt","w")
            sys.stdout = log_file
                ...
                with tqdm(desc = file_name, total = no_of_lines) as pbar:
                    ...
                    for line in inf:
                        ...
                        try:
                            ...
                            with open("assets/processed_dataset/"+self.config["asset_name"]+"_corona_tweets_"+i+"_counters.pickle", 'wb') as handle:
            ...
            with open("assets/processed_dataset/"+self.config["asset_name"]+"_corona_tweets_"+i+"_processed_data.pickle", 'wb') as handle:
